...
v2.9.3）。实现与它冲突时以它为准，除非你先改它并写明理由。
...
**不要用 `git add -A`，按路径加。** 2026-09-04 一天里它出了三次事故：两次把
`compose.override.yaml`（写着「不要提交」、`.gitignore` 里也有它，但因为已经被
跟踪所以那条规则从来没生效）扫进无关的提交；一次把一个功能的**实现**扫进了另一
条分支的提交，于是 PR 上只有测试——**CI 因此红得完全正确**，而本地全绿。
前两次是噪音，第三次是「测试和实现不在一起」，那正是这份文件开头那条 TDD 约定
要防的东西。

## 跑不动的时候

**`docker build` 突然拉不到基础镜像，先别怀疑网络。** 这台开发机的代理开着
fake-IP：域名解析成 `198.18.x`，宿主机自己的 TUN 接管，所以 `curl` 通；而
**OrbStack 虚拟机的包不走那个 TUN**，守护进程拿到同一个假 IP 就超时。
判据是三者分开测：宿主机 `curl` 通、容器 `nc :443` 通、`docker pull` 超时。

**这个坑一直在，平时被缓存盖着**——基础镜像在本地、依赖层在 buildx 缓存里，
build 根本不碰网络。2026-09-04 清了一次磁盘（`docker builder prune` +
`docker image prune`）就露出来了，而当时的第一反应是「你的代理今天变了」——
不是，是清缓存的人（我）把它挖了出来。

绕法不用碰代理：宿主机能到 registry，所以在宿主机下载再 `docker load` 喂进去。
两个坑都会伪装成同一种症状：大 blob 会被截断（22MB/64MB 就断，curl 不一定
报错），token 几分钟就过期（之后每层拿回 157 字节的 `UNAUTHORIZED`）。
**所以每层都要按 manifest 里的摘要验过才算数**——只看「文件下下来了」，喂进去
的会是半个镜像。
