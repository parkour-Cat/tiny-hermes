...
- ~~没有证明这些审计行在浏览器里真的看得见~~ — **看见了。**2026-09-04 的心跳走查
  停了一次 scheduler 又起回来，之后有人打开控制台的审计页，确认
  `channel.long_connection.disconnected` 在页面上。这条链此前只被读代码印证过
  （行在库里、`workspace_id` 非空、API 不加默认筛选、页面原样渲染动作名），
  而读代码印证不了这件事——这份文件开头那一段说的就是它。
...
- **不声称一个进程能带多于一个长连接绑定。**SDK 的 ws loop 是模块级单例，第二个绑定
  ...
  不是支持。**
- ~~不声称建连成功之后掉线会被自动恢复~~ —— **2026-09-02 补上了活性检查**：
  `_wait_while_alive` 盯 `_down_since`（SDK 报掉线时置上、报重连成功时清掉），
  一直非空超过 `OUTAGE_GIVE_UP_SECONDS` 就放弃这一轮 `run()`，让
  `_supervised_connection` 从头重建。**仍然不声称的是那个上限值对不对**——
  五分钟是个判断，不是测量结果；也仍然没有在真实断网上验证过。
...
修掉 `_envelope_of` 之后重新部署，**隧道仍然关着**，再发一条消息。库里核对：

| 环节 | 证据 |
|---|---|
| 帧从长连接进来 | `channel_events.channel_event_id = om_x100b664d017658b4c12b9ac8fa9ab43` —— `message_id` 形态，正是修订后的键 |
| 认领 | 恰好一行 |
| 变成 Run | `run_id` 非空，`runs.status = completed` |
| 回复发出去了 | `replied_at` 非空，`reply_attempts = 1`（一次成功） |
| 隧道 | **关着**，飞书没有其他路径能送进这条消息 |

**这是这条分支唯一真正的验收，它现在有据了。**

### 6.2 这一遍值多少

走查抓到的那个信封缺陷，**七轮对抗性评审都没抓到**，后端测试全绿，它会带着绿灯合进
...
合起来：**当时一个管理员只用控制台，没法把网页渠道从零开起来。**这与本分支刚补掉的
...
**这条渠道真正的缺口不在数据链路上，在开起来的路上**——见上面两条。

> **这两条已经补掉了**（2026-09-02，`feat(web): 控制台能开起网页渠道了`）：签发方表单
> 改成「从 JWKS 地址读取 / 直接贴公钥」二选一，Agent 详情页加了「允许终端用户访问」
> 开关。补完之后又走了一遍，**全程只用控制台、没碰过一次 SQL**，Agent 真的回了话。
> 见 `2026-09-02-web-channel-switches.md`。**这一段保留原样是有意的**——它记录的是
> 当时的事实，以及这个缺陷是怎么被抓到的（不是评审读出来的，是走查撞出来的）。
