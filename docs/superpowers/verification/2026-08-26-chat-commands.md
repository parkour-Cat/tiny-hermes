...
真实飞书应用、真实用户、部署的那套栈，2026-08-29 21:06–21:37：

| 走查 | 观察到的 |
|---|---|
| `/undo` | 识别；`has_run=f`（没建 Run）；`command_receipt` 写入；回执送达 |
| `/new` | 「已经开始一段新对话。之前的 3 条消息不再进入上下文。」 |
| `/usr/local/bin` | **没被吞**，原样变成 Run 并正常完成 |
| `/undo 2`（会话里只有 1 轮） | 「已撤回 1 轮，共 20 条。」并回显原文 |

最后一条是这一遍最值钱的：请求 2 轮而只有 1 轮，回执说的是**实际**的 1。
一个把请求数字原样回显的实现，测试同样会绿，而用户会以为自己丢了 2 轮。

「1 轮 / 20 条」两个数已对着库核过，不是估的：那一批是
`user 1 + assistant 10 + tool 9`——轮数按 user 消息算，条数按实际标记的行算。
...
- **真机走查只覆盖了一个租户、一个绑定、一个 Agent（`notes-tidier`）、一个人。**
  群聊、多人、不同 Agent、不同人格都没走过。
- **`/undo` 只在「没有在飞的 Run」这一种状态下走过。** 头部停在 `waiting_approval`
  或 `paused` 时的拒绝与 `/new` 的取消路径，**只有测试覆盖，真机没走过**——而那
  恰恰是最终评审判为 blocking 的那条路径。
- **回执与撤回不在同一个事务里的那个窗口，没有真机验证过。**
- **没有走过飞书那侧的失败**：发送失败、令牌过期、限流之后回执会怎样，没观察过。
- **控制台与 chat-web 的「已撤回」标记只有测试证据**，没有在部署的界面上肉眼看过。
