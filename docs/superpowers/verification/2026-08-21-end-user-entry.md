...
> **后续（2026-08-22）。** §10 已经做完并有自己的验收记录
> （`2026-08-22-end-user-controls.md`）：列表接口有了，聊天界面能显示并答复待答确认，
> e2e 里一个人在界面上点了「同意」而 Run 接着跑完。**§27.2 场景 6 到此可以记成绿的。**
> 但 §4.6 的 `Run 暂停、继续与取消` 仍然只做了取消这三分之一——`暂停` 与 `继续`
> 有意不建，并由两条断言 404 的测试钉住。那一行**没有**兑现完。
