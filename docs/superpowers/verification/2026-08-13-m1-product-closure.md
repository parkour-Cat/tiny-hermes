...
| 13 | Run §24.1 on the Linux reference shape (8 vCPU, 16 GiB, 50 ms delay, image cached, 100k RunEvents) | `scripts/benchmark_m1.py`; unit pins in `packages/backend/tests/unit/scripts/test_benchmark_m1.py`; host shape: `2026-08-14-m1-reference-host.md`; live run: `2026-08-14-m1-benchmark-live.md` | **not a pass.** 8 vCPU / ≈30.47 GiB, `shape_ok` true. Live drivers measured: create_run / sandbox_cold / workspace_* / next_run inside cells; run_event writes met p95/rate/loss but failed a 300.0&lt;300 clock compare; SSE 498/500 missed 5s cadence; warm p95 26s; worker queued 34.72s; API recovery 66.39s. Do not edit thresholds to make a run green. |
