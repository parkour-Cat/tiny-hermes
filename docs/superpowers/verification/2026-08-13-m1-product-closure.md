...
| 13 | Run §24.1 on the Linux reference shape (8 vCPU, 16 GiB, 50 ms delay, image cached, 100k RunEvents) | `scripts/benchmark_m1.py`; unit pins in `packages/backend/tests/unit/scripts/test_benchmark_m1.py`; later hosts: `2026-08-14-m1-reference-host.md` | **script exists; not a pass.** Cloud Agent is 4 vCPU. First operator host 8 vCPU / ≈15.12 GiB (`shape_ok` false). Second operator host 8 vCPU / ≈30.47 GiB (`shape_ok` true, exit 1: every live gate `not_run`). Workspace and restart drills passed on both. Do not edit thresholds to make a run green. |
