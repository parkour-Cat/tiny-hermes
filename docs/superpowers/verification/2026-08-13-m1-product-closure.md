...
| 13 | Run §24.1 on the Linux reference shape (8 vCPU, 16 GiB, 50 ms delay, image cached, 100k RunEvents) | `scripts/benchmark_m1.py`; unit pins in `packages/backend/tests/unit/scripts/test_benchmark_m1.py`; host shape: `2026-08-14-m1-reference-host.md`; live runs: `2026-08-14-m1-benchmark-live.md`, `2026-08-14-m1-benchmark-live-2.md` | **not a pass.** 8 vCPU / ≈30.47 GiB, `shape_ok` true. Run 2: create_run / run_event / sandbox_cold / workspace_* / next_run / service_recovery inside cells; SSE 500/500 missed 5s cadence (attach gap); warm p95 26.09s is leaked WorkerLease after `apply_signal(SLICE_ENDED)`, thaw after second lease ~170ms; worker queued 31.26s / 30s. Do not edit thresholds to make a run green. |
