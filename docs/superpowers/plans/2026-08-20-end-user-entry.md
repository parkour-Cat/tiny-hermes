...
## 10. 全分支评审补出的一节：终端用户自己的控制面

**这一节是计划的漏，不是实施的漏。** 前九节没有任何一节被指派去建它，所以八个任务
各自都「完成」了，而 §4.6 矩阵里给终端用户的两格始终没有落点。全分支评审是第一个
能看见它的位置——单节评审看不见，因为缺的东西不在任何一节的范围里。

§4.6（v2.5，第 198–199 行）给终端用户这两格：

| 权利 | 终端用户 | 这个分支的状态 |
|---|---|---|
| Run 暂停、继续与取消 | 本人 | **没有任何路由**。控制台的 Run 控制接口对终端用户一律 403 |
| 用户确认审批 | 仅发起人本人 | 生产者建好了，**消费者到不了**：没有列表接口，没有任何终端用户响应里带得出 id，`apps/chat-web` 里搜不到 approval |

两格缺的是同一样东西：**Run 停在那里之后，本人没有一个能碰它的入口。**

这不只是权利没兑现，它还卡住抹除。`WAITING_APPROVAL` 的 Run 一直占着
`sessions.head_run_id`，而 head 只在终态转换时释放——本人既不能答确认也不能取消，
