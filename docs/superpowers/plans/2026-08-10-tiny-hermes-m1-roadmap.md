...
**细化计划：** 阶段三分三片交付——3A 真实模型（`2026-08-11-m1-real-model.md`）、
3B 沙箱执行（`2026-08-11-m1-sandbox.md`）、3C SessionWorkspace 与文件工具
（`2026-08-11-m1-session-workspace.md`）。3C 明确：`/workspace/data` 的字节与
对象上限是**检查点配额**（决定什么可以成为已提交 revision），不是宿主机磁盘
硬限制；命令可以在运行期间暂时超出，冻结扫描负责拒绝提交。
