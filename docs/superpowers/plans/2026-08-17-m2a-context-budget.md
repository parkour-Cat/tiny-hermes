...
- [ ] `packages/backend/tests/unit/runs/test_context_budget.py`：先写会失败的测试。
...
- [ ] Run 详情时间线认识这两个事件，并说出人话：裁了什么、压了哪几条。
      i18n 键写进 `zh-CN`，和 `goal_verdict` 同一处。
- [ ] `paused(context_overflow)` 的状态说明：这一条和 `paused(limit)` 一样，
      读的人需要知道自己是不是那个卡住它的人。`explain.ts` 已经有这个形状。
- [ ] 前端单测覆盖新的说明文案，`explain.test.ts` 加一条。
