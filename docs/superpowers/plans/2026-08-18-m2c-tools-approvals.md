...
- [x] 数据模型与迁移 `0023`：`model_pricing_versions`（币种、输入/输出/缓存单价、
      生效时间、维护人）。**未配置价格 ≠ 价格为零**：
      未配置时显示「费用未知」，管理员可以显式把本地模型价格设为 0，
      这与「未知」是两个不同状态，两者各有一条测试。
- [x] `Run` 创建时固定引用当时的 `model_pricing_version_id`（§12.4）。
      运行中改别名或改价格不会重算已经开始的 Run——
      安全阀和用量记录因此可以追溯。
      **货币硬上限放在工作空间上，不在 `AgentLimits` 里**：那个模型在被哈希的
      spec 内部，加一个字段会给每个已发布版本的规范化文档多一个键，
      把这个平台写过的每一个内容哈希都改掉。上限也本来就是运维的决定
      而不是 Agent 作者的，两个理由指向同一处。工作空间的值在 Run 创建时
      抄到 Run 上，所以正在被度量的 Run 脚下的阀门不会移动。
- [x] 预检与修正：每次模型调用前按预计输入与最大输出做预检，
