...
- [ ] 数据模型与迁移 `0022`：`model_pricing_versions`（币种、输入/输出/缓存单价、
      生效时间、维护人）。**未配置价格 ≠ 价格为零**：
