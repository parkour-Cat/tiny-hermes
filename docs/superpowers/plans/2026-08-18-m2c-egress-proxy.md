...
- [x] `container_policy.py`：`network_mode` 从 `none` 变成一个**只能到达 proxy 的
      专用网络**。沙箱仍然没有到互联网的直连，也仍然没有到平台内网其他服务的路。
- [x] 容器内注入 `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY`，让常见运行时默认走 proxy；
      但**不依赖它**——网络本身就到不了别处，环境变量只是让工具的错误信息好看一点。
- [x] `tests/integration/sandbox/`：真容器里的断言，但**不是用 `curl`**——
      运行时镜像里没有 `curl` 也没有 `nc`，为了测试给它加一个，就是给每个 Agent
      手里塞一个网络工具。探针改成 bash 内建的 `/dev/tcp`，断的是否定的那两条：
      无网络的沙箱够不到任何地方，挂在 egress 网络上的沙箱**仍然**够不到外网。
      这两条才是整套安排的立足点——把容器接上一张网络之所以安全，是因为那张网络
      哪儿也去不了。肯定的那半（沙箱经 proxy 真的取到东西）留给 compose，
      理由写在测试文件里：那需要网络另一端真的有个 proxy，而两端都真实存在的
      地方是 compose，不是这套单机测试。
- [x] 冻结与销毁路径要一并撤掉网络入口：一个被冻结的沙箱不能再发起新连接
