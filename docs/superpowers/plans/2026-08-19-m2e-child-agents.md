...
- [x] 父传子:父把选定文件存成 Artifact 并创建**只读授权**;
      子 Run 读的是那份授权,不是父的目录。授权是 `artifact_grants` 一行,
      **按 Run 发**而不是按 Agent 或 Session——同一个 Agent 后面那个 Run
      不该能打开没人交给这一件活的东西。父自己读不到的文件传不下去,
      而且是**整次委派拒掉**,不是把那一个文件悄悄丢掉。
- [x] 子传父:子的产物上传对象存储后,由平台授予父 Run 读取权。
      在投递结果的同一个事务里发,所以结果里点名的 id 都是父真能打开的。
- [x] 一条测试断言父子**没有共享的可写挂载**——红线三,
      断言的是"没有这条路"而不是"这条路被拒了"。
- [x] 补的一件计划没写但少不了的:`artifact.read`。授权不给读的路子
      就是一行没用的记录。它只按**本 Run 的**授权判,"不存在"和"不是你的"
      回同一句话——分开说,等于让 Agent 靠试探把别人的 id 摸出来。

## 6. 控制台：最小任务树

- [x] Run 详情显示这个 Run 的父与子（有的话），子 Run 可点进去。
      完整任务树视图是 M3,这里只要**能看出这是一棵树**。
      子任务列表来自 Run 文档新增的 `children`,不另开接口——
      「这是不是一棵树」本来就是 Run 的一部分,问两次会先画出 Run
      再补上树。没有子任务就整张卡片不出现。
- [x] `explain.ts` 加 `child_runs` 等待的人话:说清楚它在等谁、等几个、
      `all` 还是 `any`,以及"它自己不会醒"。`any` 那句明说其余会被取消,
      包括马上就要跑完的——这个代价要写在人看时间线的地方,
      而不是让人从账单上发现。
- [x] 两个 locale 的 i18n。
