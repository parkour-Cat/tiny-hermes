...
- **例外只有一个：`/new` 可以结束一个「停住的」队首 Run**（`waiting_approval`、
  `waiting_external`、`paused`）。上游那三个 issue 拆的都是**真的在跑**的工作；
  停住的 Run 没有工具在执行，它等的是一个人或一个外部事件，取消它不会把某个副作用
  截在半路。走的是现成的 `RunCoordination.cancel_end_user_run`，合法转移表仍归
  `RunStateMachine`。
  **`/undo` 对停住的队首照样拒绝。** 不对称是故意的：`/new` 是逃生口——下面那条
  「阻塞卡片里写明 `/new`」的决定正是对着这几种状态做的，不给它这个权限那句话就是
  假的；`/undo` 是对已经落定的历史动刀，没有理由替用户放弃一个他没说要放弃的 Run。
  **代价必须写明：只结束队首那一个 Run。**排在它后面的照常被提上来，所以 `/new`
  之后仍可能收到一条来自旧消息的答复，而那一轮的历史已经被撤掉了。本版明知而未做。
  取消失败（非法转移、状态版本冲突）则**什么都不撤**，按忙拒绝——照样撤等于给用户
  一段新对话加一个还能醒进来的 Run。
...
- **忙的判定**必须同时覆盖「队首 Run 未结束」与「后面还有排队的 Run」两种。
  ...
  `/undo` 对三档一律拒绝；`/new` 只对前两档拒绝，第三档结束那个队首再撤（§3.2）。
