# tiny-hermes 产品与系统设计 v2.8

> 日期：2026-08-30  
> 版本：v2.8  
> 状态：已确认；M1 与 M2 已交付，可作为 M3 实施依据（§7.4.2 的 v2.7 改动尚未实现）  
> 目标版本：首个企业预览版  
> 许可证方向：Apache-2.0
...
v2.8 只改 **§7.4.2**，补上 v2.7 留下的一个洞：摘要生成是一次真实的模型调用，会在真实
端点上花钱，但那次调用的 usage 与费用从未计入过 Run 的预算范围，也从未出现在事件流
里——一个摘要总是失败的 Session 会每轮都白付一次没人算得到的钱，那次调用本身也永远
不可能把 Run 推到 §12.4 的费用上限。现在它按摘要端点自己的价格计费（Agent 声明了独立
摘要端点时不按主模型的价），只要 Provider 回了 usage 就算，不论 stop_reason 是否
completed；累计的数字要能在下一轮的预检里读到，并单独记一条 RunEvent，带上端点、模型、
token 数和费用。
...
**摘要生成是一次真实的模型调用，按 §12.4 记账。** 它的 usage 与费用计入该 Run 所在
RunBudgetScope 的 consumed_tokens/consumed_cost——按摘要端点自己的价格，Agent 声明了
独立摘要端点时不按主模型的价格算；只要 Provider 回了 usage 就算，不论那次调用的
stop_reason 是不是 completed，一次拒绝或答非所问同样是在真实端点上花了钱的调用，只有
从未打到 Provider 的调用（超时前就抛出异常）不计。累计后的数字要能在下一轮的 §12.4 预检
里读到，使这次调用和其他任何一轮一样能把 Run 推到费用上限——一个摘要总是失败的会话不能
每轮白付一次钱却不被算进任何上限。**这次调用单独记一条 RunEvent**，带上端点、模型、
token 数和费用，不并入 `CONTEXT_COMPACTED`：后者说的是「这一轮压缩发生了」，前者说的是
「那次生成调用花了多少」，复用一份已存摘要时压缩照常发生但不产生新的调用，两件事分开
才不会互相冒充。
