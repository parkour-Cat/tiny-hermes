# tiny-hermes 产品与系统设计 v2.5

> 日期：2026-08-20  
> 版本：v2.5  
> 状态：已确认；M1 与 M2 已交付，可作为 M3 实施依据  
> 目标版本：首个企业预览版  
> 许可证方向：Apache-2.0
...
v2.5 只改一处：§4.5 补全终端用户身份的四个未决问题，它们此前只写了「两个身份体系」，
没写这个体系怎么运作，而 M3 的多数条目都压在上面。四条决定是身份来源、Agent 分配、
会话可见性和聊天界面形态，见 §4.5.1 至 §4.5.4。§4.6 的矩阵不变，只在 §4.5.3 补一句
说明「查看状态」与「查看内容」的区别——那个区别矩阵在 Run 那一行已经做过。
v2.5 替代 v2.4 中 §4.5 的对应定义，其余部分不受影响。
...
这一区分不是新规则：§4.6 在 Run 那一行已经把「控制」与「可查看状态」分开。此处只是
声明它同样适用于终端用户会话——开发者能够运维一个 Agent，而不能读到用户对它说了什么。

因此工作空间范围的会话与消息查询必须按调用主体收窄。这是既有接口的一处收口，且必须
与终端用户身份**同时**落地。
...
| 工作空间治理审批 | 是 | 是 | 否 | 否 | 否 |
| 本人会话与私有记忆的查看、更正、删除和导出 | 依法代办并审计 | 本空间代办并审计 | 否 | 否 | 本人 |
...
压缩结果必须记录覆盖的消息范围和原始引用。压缩失败后保留原文；若保留原文又无法装入窗口，Run 进入 `paused(context_overflow)`，不能继续请求 Provider，也不能静默删除中间消息。
