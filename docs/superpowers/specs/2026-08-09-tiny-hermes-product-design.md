...
v2.9.3 只改 **§7.4.2**，补上一条它从未说过的话：一次不会让上下文变小的压缩不算
压缩，不得采用。

起因是线上第一条 `/compact`。它压掉两条短消息，摘要调用花了 1,727 个 token，估算
省下 0，回执照样报「已压缩」——上下文不但没变小，还大了一点。§7.4.2 原来的判据
只有「装不装得下」和「边界对不对」，一份比原文还长的摘要两条都过得了。

同时定下手动压缩的三处行为：手动压缩取最大那个仍然装得下的边界，自动压缩取最小的
（不写这一条就会得到一个和对话长度无关的结果——线上那次是十七条历史只压掉两条）；
不花模型调用的结构摘要已经省不出东西时不再发那次摘要调用（结构摘要永远比模型摘要
短，所以这是可推的，不必先花钱试）；「看过之后主动不压」与「压缩失败」分开记录、
分开措辞，因为「稍后再试一次」对前者是假话。
...
**一次不会让上下文变小的压缩不算压缩，不得采用。**摘要（模型写的或结构的）比它
替掉的那段历史还长时，采用它会让这一轮发出去的 token 变多——花了钱，把事情做反了。
判据是那次压缩估算省下的 token 必须为正；这不是一个可配的阈值，「等于零」本身就是
「没省到」。

**手动压缩尽量多压，自动压缩压到够用为止。**两者用的是同一个边界搜索，方向相反：
自动压缩是为了让这一轮装得下，取**最小**那个够用的边界——少改一段历史就少作废一次
provider 的前缀缓存；`/compact` 是有人明说了「现在压」，取**最大**那个仍然装得下的
边界。这条要写明，是因为不写就会得到一个和对话长度无关的结果：`threshold` 被置零
之后最小的边界当场就满足条件，于是一段十七条历史的会话只压掉最老的两条。

这一条同时改变**手动压缩**（`/compact`）的两处行为：结构摘要（不花模型调用）已经
省不出东西时，模型摘要更不会——所以这时一次摘要调用都不发；而这种「看过之后主动
不压」必须和「压缩失败」分开记录、分开措辞。失败可以稍后再试，这一种再试同样不会
成，只会再白花一次摘要调用。

写下这一条的起因是线上第一条 `/compact`：它压掉两条短消息，摘要调用花了 1,727 个
token，估算省下 0，回执照样报「已压缩」。

**估算值只用于决定发什么，不作为报给人看的测量值。**`estimate_tokens` 按定义是上界
（宁可高估，免得算出来装得下、实际超窗），用在「该不该压、装不装得下」上是对的；
把同一个数当成「你省了多少」报给用户，报的是一个从设计上就偏高的量。2026-09-03 对
过一次真机数据：一次压缩报「每一轮少发 3,089 个 token」，provider 数同一批内容是
1,605（这个数还包含提示词本身）。回执改为只报**数出来的**条数。
