# tiny-hermes 产品与系统设计 v2.9

> 日期：2026-08-30  
> 版本：v2.9  
> 状态：已确认；M1 与 M2 已交付，可作为 M3 实施依据（§12.1 的 v2.9 让位规则尚未实现）  
> 目标版本：首个企业预览版  
> 许可证方向：Apache-2.0
...
**`continue` 让位于排队中的用户消息。** 当同一 Session 中已有排队的 pending Run 时，
判为 `continue` 的 Run 不得继续下一轮，而是以 `completed` 结束、让出队首，使那条消息
立即得到处理。
