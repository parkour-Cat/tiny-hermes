...
class ChannelImageSource:
    ...
    async def data_url_for(self, reference: str, session_id: UUID) -> str:
        ...
        if target is None or target.app_id is None or target.app_secret_ref is None:
            # No binding, or a receive-only one. Raised rather than returning
            # nothing: `resolve_images` turns this into a failed round with a
            # stated reason, and the alternative is a question about a picture
            # sent without the picture.
            raise LookupError(
                f"session {session_id} has no binding that can fetch images"
            )
        secret = await self._resolve_secret(target.app_secret_ref)
