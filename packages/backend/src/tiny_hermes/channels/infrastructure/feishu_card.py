...
def command_receipt_text(receipt: CommandReceipt) -> str:
    ...
    if receipt.command == "compact":
        # `/compact` 走到这里只有一种结果：`nothing`。真的要压时它**不发命令
        # 回执**——建一个压缩 Run，压完之后由 `reply.py` 发那句话
        # （`ingestion.py` 那一支返回的是 `receipt=None`）。
        #
        # 这里原来还有一句「记下了。下一条消息之前，会先把这段对话的旧历史压成
        # 摘要。」，以及一段注释说这条命令「做的是打标记，压缩发生在下一轮真正
        # 开始前」。那是改成「发完就压」之前的设计，两样都已经不成立；那句话正是
        # 用户第一次真机走查看到的、并且反问「『记下了』是什么鬼」的那一句。
        # 删掉而不是留着当保险：一句到不了、又和当前设计矛盾的措辞，下一个读它
        # 的人只会照着它去理解这条命令。
        if receipt.outcome == "nothing":
            # 对 `/compact`，「没什么可压」是一句让人放心的话，不是失败。和
            # `/undo` 的同名结果合并会让人以为命令没生效，于是再发一次。
            return "这段对话还不长，暂时没有需要压缩的历史。"
        # 兜底。今天到不了，而它说的话对将来任何一种「命令这一刻就发回执」的
        # 改法都仍然成立：不承诺压缩会发生，也不声称它已经发生。
        return "收到 /compact。压缩的结果会在压完之后单独发一条。"
