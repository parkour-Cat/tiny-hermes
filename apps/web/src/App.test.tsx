...
test("the login page offers the agent workbench as well as the console", async () => {
  server.use(
    http.get("/api/v1/auth/me", () =>
      HttpResponse.json({ code: "unauthenticated" }, { status: 401 }),
    ),
  ...
  expect(await screen.findByRole("link", { name: "使用智能体" })).toHaveAttribute("href", "/chat");
});

test("signing in from the workbench returns there, not to the console", async () => {
  let signedIn = false;
  server.use(
    http.get("/api/v1/auth/me", () =>
      signedIn ? HttpResponse.json(ADMIN) : HttpResponse.json({ code: "unauthenticated" }, { status: 401 }),
    ...
    http.get("/api/v1/workspaces", () =>
      HttpResponse.json([{ id: "11111111-2222-4333-8444-555555555555", name: "Acme", status: "active" }]),
    ),
  ...
  expect(await screen.findByRole("heading", { name: "和智能体说话" })).toBeInTheDocument();
  expect(screen.queryByRole("button", { name: "新建工作空间" })).not.toBeInTheDocument();
});
