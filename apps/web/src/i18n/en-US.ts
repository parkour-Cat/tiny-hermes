...
export const enUS: Record<MessageKey, string> = {
  ...
  contextSummaryBilledNote:
    "That summarization call spent {tokens} tokens ({inputTokens} in, {outputTokens} out), billed {cost} {currency} against this Run's budget, and counted as one model call — real usage, not an estimate. Answered by model {model}, endpoint {endpoint}.",
  contextSummaryBilledUnknownCostNote:
    "That summarization call spent {tokens} tokens ({inputTokens} in, {outputTokens} out) and counted as one model call; its cost is unknown — the endpoint that answered has no price configured, which makes this Run's cost total read as unknown from here on, never a 0. Answered by model {model}, endpoint {endpoint}.",
